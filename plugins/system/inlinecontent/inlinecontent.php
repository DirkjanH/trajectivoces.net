...
class PlgSystemInlineContent extends JPlugin
{
	...
	protected $input;
	protected $option;
	protected $context = array('module','fields');
	protected $types = array('module','fields');
    ...
    public function onAfterRoute()
	{
		...
		{	
			...
			$this->app->input->set('ark_inine_enabled',true);
            $this->app->input->set('ark_inine_context_enabled',true);
	    }
       
        JPluginHelper::importPlugin('inlinefields');	
 	...
 	public function onAfterDispatch()
	{
       ...
       //Inline editing is only enabled for frontend editing	
        ...
        /*
        $reflectionClass = new ReflectionClass('JEditor');
        $reflectionProperty = $reflectionClass->getProperty('_editor');
        $reflectionProperty->setAccessible(true);
        $testEditor = $reflectionProperty->getValue(JEditor::getInstance());
        $reflectionProperty->setAccessible(false);
        */

        $getEditor = function() { return $this->_editor;};
        $testEditor = $getEditor->call(JEditor::getInstance());
		...
		if(!$view )
        {
            ...
            if($active && $active->query['option'] == $this->option)
            {
    ...
    public function onAfterRender()
	{
		...
		if ($this->app->isClient('site') && $this->isEnabled)
		{
            $user = JFactory::getUser();
		   ...
		   //if user is guest lets bail
			...
			$body = preg_replace('/="([^"]*?){div class=__ARKQUOTE__editable__ARKQUOTE__ data-id=__ARKQUOTE__(\d*)__ARKQUOTE__ data-context=__ARKQUOTE__(?:'.implode('|',$this->context).')__ARKQUOTE__ data-type=__ARKQUOTE__title__ARKQUOTE__ data-itemtype=__ARKQUOTE__(?:'.implode('|',$this->types).')__ARKQUOTE__ data-versiontype=__ARKQUOTE__(\w*)__ARKQUOTE__  contenteditable=__ARKQUOTE__true__ARKQUOTE__ style=__ARKQUOTE__display:inline;__ARKQUOTE__}([^"]*?){\/div}/',
					'="$1$3',$body);					
		...
		elseif($this->app->isClient('site') && $this->option == 'com_config')
		{
			// Get the response body
			$body = $this->app->getBody();
