...
class FeedView extends AbstractView {
	...
	public function display( $tpl = null ) {
		...
		$category = AllVideoShareQuery::getCategory( $app->input->getString( 'slg' ), 'alias' );
		if ( ! $category ) {
		...
		$route = 'index.php?option=com_allvideoshare&view=category&slg=' . $category->slug . '&format=feed';
		...
		foreach ( $items as $item ) {
			...
			$feeditem->date        = $date;
			$feeditem->category    = $category->name;
