...
class ARKControllerList extends ARKController
{
	...
	function save()
	{
        ...
        $model = $this->getModel('EditPlugin','ARKModel');
                
        $form = $model->getPluginForm($post, false);
		...
		if ($validData === false)
		{
            $this->setRedirect( JRoute::_( 'index.php?option=com_arkeditor&task=list.edit&cid[]='. $row->id, false ));
        }
        
        $extraData = array();

        foreach($validData['params'] as $key=>$value)
        {
     
            if(($data = $input->get($key.'_text',array(),'string')))
            {
                $extraData[$key.'_text'] =   $data;
            }
            $extraData[$key] = $value;
        }

           
        $validData['params']  = $extraData;
        
        if (!$row->bind($validData)) {
