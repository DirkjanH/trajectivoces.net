...
class ARKViewtoolbar extends JViewLegacy
{
	...
	function display( $tpl = null )
	{
		...
		$this->showSideBar =  version_compare(JVERSION, '4.0', '<' ); 
		...
		if(version_compare(JVERSION, '4.0', 'ge' ))
		{
			...
			$attr = ' placeholder="' . JText::_('JGLOBAL_TYPE_OR_SELECT_SOME_OPTIONS') . '" ';
			...
			$list = JHTML::_('select.genericlist',  $allcomponents, 'components[]', ' size="10" multiple', 'value', 'text', $components);
				
			$lists['components'] = '<joomla-field-fancy-select'.$attr.'>'.$list.'</joomla-field-fancy-select>';
		}
		else
			$lists['components'] = JHTML::_('select.genericlist',  $allcomponents, 'components[]', ' size="10" multiple', 'value', 'text', $components);
				
		$this->lists =	$lists;
		$this->toolbar =  $this->item;
		$this->total = $total;
	...
	protected function addToolbar()
	{
		...
		JToolBarHelper::title( JText::_( 'COM_ARKEDITOR_SUBMENU_LAYOUT_NAME' ) .':' . chr( 32 ) . $this->item->name, 'list-view' );
