...
class pkg_arkeditorInstallerScript
{
	...
	private $templatextd_plugins = array
	(
		'arkeditor'
	);
	...
	private $filesystem_plugins = array
	(
	    'ark'
    );
	public function preflight($type, $parent)
	{
		...
		{
			$app->enqueueMessage(JText::sprintf( 'JLIB_INSTALLER_MINIMUM_PHP', '5.6').$hideMessage, 'error');
			return false;
		}
		...
		$this->minimum_joomla_release = (string) $parent->getManifest()->attributes()->version;   
	   
		if( version_compare( $jversion->getShortVersion(), $this->minimum_joomla_release, 'lt' ) ) 
		{
			$app->enqueueMessage(JText::sprintf( 'COM_ARKEDITOR_CUSTOM_INSTALL_NOT_JOOMLA_PRIOR', $this->minimum_joomla_release ). $hideMessage, 'error');
			return false;
		}
	...
	public function postflight($parent)
	{
		...
		//Publish template xtd plugins

		$query = $db->getQuery(true);
		$query->update('#__extensions')
			->set('enabled = 1')
			->where('folder = '.$db->quote('template-xtd'))
			->where('element IN ('. $db->quote(implode($db->quote(','),$this->templatextd_plugins),false).')');
		...
		$query->update('#__extensions')
			->set('enabled = 1')
			->set('ordering = 100')
			->where('folder = '.$db->quote('filesystem'))
			->where('element IN ('. $db->quote(implode($db->quote(','),$this->filesystem_plugins),false).')');

		$db->setQuery($query);
		if(!$db->execute())
			$app->enqueueMessage( 'Failed to publish filesystem plugins for ARK' );	
