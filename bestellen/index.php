...
//		foreach ($concert as $u) if ($u['uitverkocht']) $allesuitverkocht += 1;
	  	if (is_array($concert) AND count($concert) > 0) {
				...
				if (isset($aantal_vol)) echo $aantal_vol; ?>">
					...
					<div class="w3-third" <?php if (!(isset($c[ 'prijs_red']) AND $c[ 'prijs_red']> 0)) echo 'style="display: none;"'; ?>>
						<div <?php if (!(isset($c[ 'prijs_red']) AND $c[ 'prijs_red']> 0)) echo 'class="w3-grayscale w3-grey"'; ?>>
							<?php if (isset($c['txt_red']) AND $c['txt_red'] != '') echo '<label class="w3-label">aantal '. $c['txt_red'].':</label>'; else echo '<label class="w3-label">' . $txt['CJP'] . '</label>';?>
			...
			<div class="<?php if ($allesuitverkocht) echo 'onzichtbaar'; ?> w3-panel panelkleur"><?php echo $txt['werkwijze']; ?></div>
		  ...
		  <div class="w3-row">
					<div class="w3-third w3-panel w3-leftbar w3-border-green">
						<p>
							...
							<input class="blokje w3-input w3-border" name="aanbrenger" type="text" id="aanbrenger" size="20" value="<?php 
			...
			<div class="w3-third w3-panel w3-leftbar w3-border-green">
					  ...
					  <label class="w3-label w3-validate"><?php echo $txt['anders']; ?><input class="blokje w3-input w3-border" name="anders" type="text" value="<?php 
                                if (isset($_POST['anders'])) echo stripslashes($_POST['anders']); ?>" size="20"></label></div></p>

            <div class="w3-panel">
              <label class="w3-label w3-validate">Eventuele opmerkingen: <br><input class="blokje w3-input w3-border" name="opmerkingen" type="textarea" value="<?php if (isset($_POST['opmerkingen'])) echo stripslashes($_POST['opmerkingen']); ?>" rows="2" cols="100" ></label>
            </div>
		    </div>
			<div class="w3-container">
                <p>Klik nu op 'verzenden'. Je wordt na het klikken doorgeleid naar de betaalpagina van Mollie, waarop je via iDeal kunt betalen. Daarna zie je een bedankpagina en ontvang je een mail met een QR-code, die je bij het concert aan de kassa kunt tonen.</p>
                <input class="w3-btn w3-green" <?php if (isset($allesuitverkocht) and $allesuitverkocht === true) echo 'DISABLED '; ?>name="submit" type="submit" value="<?php echo $txt['verzenden']; ?>">
				<p>P.S. het kaartverkoopsysteem is vernieuwd. Mocht je een probleem tegenkomen, stuur dan SVP even een mailtje aan <a href="mailto:organisatie@trajectivoces.net">organisatie@trajectivoces.net</a> of een appje aan 0619 224 758.</p>
			</div>
		</FORM>
	</div>
</body>
