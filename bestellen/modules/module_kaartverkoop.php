...
foreach ($concerten as $row) {
  ...
  if ($row['prijs_kind'] == (int) 0) $row['euro_kind'] = 'gratis';
...
if (isset($_POST["submit"]) && $_POST["submit"] == "verzenden") {
  ...
  if ($error) echo $fout;
    ...
    if (isset($eerdere_bestelling) and $eerdere_bestelling > 0) {
      ...
      $insertSQL = sprintf(
        "INSERT INTO {$tabel_reserveringen} (reserveringnr, voornaam, tussenvoegsel, achternaam, plaats, telefoon, email, concertId, aantal_vol, aantal_red, aantal_kind, totaal, publiciteit, aanbrenger, flyers, anders, opmerkingen, random_id) 
    ...
    if ($succes) try {
      if (isset($totaal) and $totaal > 0) {
        $payment = $mollie->payments->create([
          "amount" => ["currency" => "EUR", "value" => $totaal_string],
